...
void pannellosolare()
{
	...
	std::cout << "Plamp: (" << Plamp << "+-" << sPlamp << endl;
...
//----------------
	...
	for(int i = 0; i < nmisure; i++)
	{
		sd[i] = sqrt(pow(sd1, 2) + pow(sd2[i], 2));
	}
	...
	for(int i = 0; i < nmisure; i++)
	{
		Ramp_d[i] = Vvolt_d[i] / Iamp_d[i];
	};
	//mV / mA - Ohm = Ohm
	float Rp_d[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		Rp_d[i] = Vp_d[i] / Iamp_d[i] - Ramp_d[i];
	};
	//mV / Ohm = mA
	float Icc_d[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		Icc_d[i] = Vp_d[i] / Rp_d[i];
	};
	//mV * mA = µW
	float Ppannello_d[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		Ppannello_d[i] = Vp_d[i] * Icc_d[i];
	};
	...
	for(int i = 0; i < nmisure; i++)
	{
		sRamp_d[i] = sqrt(pow(Vvolt_d[i], 2) / pow(Iamp_d[i], 4) * pow(sIamp_d[i], 2) + 1 / pow(Iamp_d[i], 2) * pow(sVvolt_d[i], 2));
	}

	float sRp_d[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sRp_d[i] = sqrt(1 / pow(Iamp_d[i], 2) * pow(sVp_d[i], 2) + pow(Vp_d[i], 2) / pow(Iamp_d[i], 4) * pow(sIamp_d[i], 2) + 1 * pow(sRamp_d[i], 2));
	}

	float sIcc_d[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sIcc_d[i] = sqrt(pow(Vp_d[i], 2) / pow(Rp_d[i], 4) * pow(sRp_d[i], 2) + 1 / pow(Rp_d[i], 2) * pow(sVp_d[i], 2));
	}

	float sPpannello_d[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sPpannello_d[i] = sqrt(pow(Vp_d[i], 2) * pow(sIcc_d[i], 2) + pow(Icc_d[i], 2) * pow(sVp_d[i], 2));
	}
	...
	for(int i = 0; i < nmisure; i++)
	{
		Ramp_f[i] = Vvolt_f[i] / Iamp_f[i];
	};
	//mV / mA - Ohm = Ohm
	float Rp_f[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		Rp_f[i] = Vp_f[i] / Iamp_f[i] - Ramp_f[i];
	};
	//mV / Ohm = mA
	float Icc_f[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		Icc_f[i] = Vp_f[i] / Rp_f[i];
	};
	//mV * mA = µW
	float Ppannello_f[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		Ppannello_f[i] = Vp_f[i] * Icc_f[i];
	};
	...
	for(int i = 0; i < nmisure; i++)
	{
		sRamp_f[i] = sqrt(pow(Vvolt_f[i], 2) / pow(Iamp_f[i], 4) * pow(sIamp_f[i], 2) + 1 / pow(Iamp_f[i], 2) * pow(sVvolt_f[i], 2));
	}

	float sRp_f[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sRp_f[i] = sqrt(1 / pow(Iamp_f[i], 2) * pow(sVp_f[i], 2) + pow(Vp_f[i], 2) / pow(Iamp_f[i], 4) * pow(sIamp_f[i], 2) + 1 * pow(sRamp_f[i], 2));
	}

	float sIcc_f[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sIcc_f[i] = sqrt(pow(Vp_f[i], 2) / pow(Rp_f[i], 4) * pow(sRp_f[i], 2) + 1 / pow(Rp_f[i], 2) * pow(sVp_f[i], 2));
	}

	float sPpannello_f[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sPpannello_f[i] = sqrt(pow(Vp_f[i], 2) * pow(sIcc_f[i], 2) + pow(Icc_f[i], 2) * pow(sVp_f[i], 2));
	}
	...
	for(int i = 0; i < nmisure; i++)
	{
		Ppannello[i] = Ppannello_d[i] - Ppannello_f[i];
	};
	...
	for(int i = 0; i < nmisure; i++)
	{
		sIcc[i] = sqrt(pow(sIcc_d[i], 2) + pow(sIcc_f[i], 2));
	};

	float sVp[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sVp[i] = sqrt(pow(sVp_d[i], 2) + pow(sVp_f[i], 2));
	};

	float sPpannello[nmisure];
	for(int i = 0; i < nmisure; i++)
	{
		sPpannello[i] = sqrt(pow(sPpannello_d[i], 2) + pow(sPpannello_f[i], 2));
	};

	std::cout << "Icc_d[0] " << Icc_d[0] << endl;
	std::cout << "sIcc_d[0] " << sIcc_d[0] << endl;

	std::cout << "Icc_f[0] " << Icc_f[0] << endl;
	std::cout << "sIcc_f[0] " << sIcc_f[0] << endl;

	std::cout << "Vp_d[0] " << Vp_d[0] << endl;
	std::cout << "sVp_d[0] " << sVp_d[0] << endl;

	std::cout << "Vp_f[0] " << Vp_f[0] << endl;
	std::cout << "sVp_f[0] " << sVp_f[0] << endl;

	std::cout << "sPpannello[0] " << sPpannello[0] << endl;


	std::cout << "sPpannello_d[0] " << sPpannello_d[0] << endl;
	std::cout << "sPpannello_f[0] " << sPpannello_f[0] << endl;
	std::cout << "sPpannello[0] " << sPpannello[0] << endl;
	...
	/*for(int j = 0 ; j < nmisure ; j++){std::cout
	...
	out << "Ramp_d" << endl;
	for(auto & value : Ramp_d)
	{
		out << fixed << showpoint << setprecision(7) << value << endl;
	}

	out << "sRamp_d" << endl;
	for(auto & value : sRamp_d)
	{
		out << fixed << showpoint << setprecision(7) << value << endl;
	}
	...
	for(auto & value : Ppannello_d)
	{
		out << fixed << showpoint << setprecision(7) << value / 1e6 << endl;
	}
	out << endl;

	out << "sPpannello_d" << endl;
	for(auto & value : sPpannello_d)
	{
		out << fixed << showpoint << setprecision(7) << value / 1e6 << endl;
	}
	...
	out << "Ramp_f" << endl;
	for(auto & value : Ramp_f)
	{
		out << fixed << showpoint << setprecision(7) << value << endl;
	}

	out << "sRamp_f" << endl;
	for(auto & value : sRamp_f)
	{
		out << fixed << showpoint << setprecision(7) << value << endl;
	}
	...
	for(int i = 0; i < nmisure_320; ++i)
	{
		Rcarico_320[i] = Ramp_320 + Rvar_320[i];
	}
	float sRcarico_320[nmisure_320];
	for(int i = 0; i < nmisure_320; ++i)
	{
		sRcarico_320[i] = sqrt(pow(sRamp_320, 2) + pow(sRvar_320[i], 2));
	};
	...
	for(int i = 0; i < nmisure_320; i++)
	{
		Rp_320[i] = Vp_320 / Iamp_320[i] - Rcarico_320[i];
	}
	//mV / Ohm = mA
	float Icc_320[nmisure_320];
	for(int i = 0; i < nmisure_320; i++)
	{
		Icc_320[i] = Vp_320 / Rp_320[i];
	}
	//mV * mA = µW
	float Ppannello_320[nmisure_320];
	for(int i = 0; i < nmisure_320; i++)
	{
		Ppannello_320[i] = Vp_320 * Icc_320[i];
	}

	float Pcarico_320[nmisure_320];
	for(int i = 0; i < nmisure_320; ++i)
	{
		Pcarico_320[i] = Iamp_320[i] * Vvolt_320[i];
	}
	...
	for(int i = 0; i < nmisure_320; i++)
	{
		sRp_320[i] = sqrt(1 / pow(Iamp_320[i], 2) * pow(sVp_320, 2) + pow(Vp_320, 2) / pow(Iamp_320[i], 4) * pow(sIamp_320[i], 2) + 1 * pow(sRamp_320, 2));
	}

	float sIcc_320[nmisure_320];
	for(int i = 0; i < nmisure_320; i++)
	{
		sIcc_320[i] = sqrt(pow(Vp_320, 2) / pow(Rp_320[i], 4) * pow(sRp_320[i], 2) + 1 / pow(Rp_320[i], 2) * pow(sVp_320, 2));
	}

	float sPpannello_320[nmisure_320];
	for(int i = 0; i < nmisure_320; i++)
	{
		sPpannello_320[i] = sqrt(pow(Vp_320, 2) * pow(sIcc_320[i], 2) + pow(Icc_320[i], 2) * pow(sVp_320, 2));
	}

	float sPcarico_320[nmisure_320];
	for(int i = 0; i < nmisure_320; ++i)
	{
		sPcarico_320[i] = sqrt(pow(Vvolt_320[i], 2) * pow(sIamp_320[i], 2) + pow(Iamp_320[i], 2) * pow(sVvolt_320[i], 2));
		std::cout << sPcarico_320[i] << endl;
	}


	out << "Iamp_320" << endl;
	for(auto & value : Iamp_320)
	{
		out << fixed << showpoint << setprecision(7) << value << endl;
	}
	out << endl;
