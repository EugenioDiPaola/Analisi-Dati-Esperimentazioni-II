...
//Vce = 3(V)
    ...
    float sIc_3[nmisure];
    for (int i = 0; i < nmisure; ++i){sIc_3[i] = Ic_3[i] * (5 / 1000) + 5 * 0.1;}
    float sIb_3[nmisure];
    for (int i = 0; i < nmisure; ++i){sIb_3[i] = Ib_3[i] * (5 / 1000) + 10 * 0.1;}

    //Ic_3/Ib_3 
    float beta_3[nmisure];
    for(int i = 0; i < nmisure; i++){beta_3[i] = Ic_3[i] / Ib_3[i] * 1000;}//per 1000 perchè Ib è in µA e Ic in mA

    //errore propagato
    float sbeta_3[nmisure];
    for (int i = 0; i < nmisure; ++i){sbeta_3[i] = sqrt((pow(Ic_3[i],2) / pow(Ib_3[i],4)) * pow(sIb_3[i],2) + (1 / pow(Ib_3[i],2)) * pow(sIc_3[i],2)) * 1000;}
...
//Vce = 5(V)
    ...
    float sIc_5[nmisure];
    for (int i = 0; i < nmisure; ++i){sIc_5[i] = Ic_5[i] * (5 / 1000) + 5 * 0.1;}

    //
    //errori datasheet Amprobe 37XR-A
    float sIb_5[nmisure];
    for (int i = 0; i < nmisure; ++i){sIb_5[i] = Ib_5[i] * (5 / 1000) + 10 * 0.1;}

    //Ic_5/Ib_5    
    float beta_5[nmisure];
    for(int i = 0; i < nmisure; i++){beta_5[i] = Ic_5[i] / Ib_5[i] * 1000;}

    //errore propagato    
    float sbeta_5[nmisure];
    for (int i = 0; i < nmisure; ++i){sbeta_5[i] = sqrt((pow(Ic_5[i],2) / pow(Ib_5[i],4)) * pow(sIb_5[i],2) + (1 / pow(Ib_5[i],2)) * pow(sIc_5[i],2)) * 1000;}
...
//Vce = 6(V)
    ...
    //errori datasheet Amprobe 37XR-A
    float sIc_6[nmisure];
    for (int i = 0; i < nmisure; ++i){sIc_6[i] = Ic_6[i] * (5 / 1000) + 5 * 0.1;}
    float sIb_6[nmisure];
    for (int i = 0; i < nmisure; ++i){sIb_6[i] = Ib_6[i] * (5 / 1000) + 10 * 0.1;}

    //Ic_6/Ib_6
    float beta_6[nmisure];
    for(int i = 0; i < nmisure; i++){beta_6[i] = Ic_6[i] / Ib_6[i] * 1000;}

    //errore propagato
    float sbeta_6[nmisure];
    for (int i = 0; i < nmisure; ++i){sbeta_6[i] = sqrt((pow(Ic_6[i],2) / pow(Ib_6[i],4)) * pow(sIb_6[i],2) + (1 / pow(Ib_6[i],2)) * pow(sIc_6[i],2)) * 1000;}
